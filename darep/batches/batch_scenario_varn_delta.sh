...
# Usage: $0 <n_reps> <exp_number> <delta> <split>
#
# $1: <n_reps>
# $2: <exp_number> (starting from 1)
# $3: <delta>
# $4: <split>= true|false
#
...
function main() {
    ...
    DELTA="$3"
    # darep scenario extension for input file
    ...
    echo "step 7 ..."
    # generate (no make)
    #${RUNNINGPROJECT}/${DAREPDIRNAME}/bin/generatemodelcode.bash
    # resave
   ...
   # 10. generate the solver, compiling all the models, using:
    ...
    #             solvermakefile (to generate makefile file)
    #             make opt:
    echo "step 10 ..."
    ...
    cd ${RUNNINGSOLVERDIR} 
    echo "Cleaning ..."
    #make -j 6 -f "makefile${MKEXTDAREP}" clean
    echo "Solver simulator: ${RUNNINGSOLVERDIR}"
    echo "Generating ..."
    make -j 6 -f "makefile${MKEXT}" opt
