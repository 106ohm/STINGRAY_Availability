...
#awk -v name="T_0" -v nofm=289 -F, 'BEGIN{pattern="^Experiment 1,Mean," name}FNR==NR && /^Experiment 1,Result Type,Name,Start Time,End Time,Mean,/{base=FNR; next} FNR==NR && $0~pattern && foundn!=1{pvidnumber=FNR-base-1; foundn=1; next}FNR!=NR && $1>=pvidnumber && $1<=pvidnumber+nofm-1{mvalues[$1]=mvalues[$1] " " $2; next}END{n=asorti(mvalues, ksorted); for (i=1; i<=n; i++){print mvalues[ksorted[i]]}}' Results_results.csv trace_Study_s187704482_e0_N1000_n100.csv
...
#tracefile="trace1.csv"
#plot for [i=1:batches] "<awk -v name=".measurename." -v nofm=".numberofmeasures." -F, -f gnuplot/mean.awk ".resultsfile." ".tracefile." " using i
...
#tracefile="trace_s187704482_e0_N1000.csv"
tracefile="trace_Study_s187704482_e0_N1000_n100.csv"
#tracefile="trace3.csv"
